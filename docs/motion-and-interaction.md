...
| `robot-cactus` | `breathe` shut and idling, so the slow wander is the motion; `flower` the whole flowering, open, held, shut again; `reach` the arms working with the corolla never more than ajar; `static` | Drag up and down to work the flowering; arrows 5%, shift 15%, Home shut and End wide. The whole column leans toward the pointer throughout and carries the arms and the flower with it. `onBloomChange` |
