...
| `airliner` | A four-engine double-deck widebody, and the first machine here you can walk all the way round: the camera goes to any azimuth and elevation, not only the four the set names. The skin is longitudinal panels, so a cutaway is a camera-relative cull that follows you round the aeroplane, and the whole thing comes apart in twelve stages — freight and decks out, five gear units, the surfaces off the wing, engines off pylons, and so back to the centre section it was built on. |
...
| `rod-pump` | The downhole end of a beam-pumped well, in section: working barrel, plunger, and the two ball valves whose sequence the whole thing turns on, with the dynamometer card drawn beside it. The set's first cutaway subject — every tubular is the cylinder it is, opened on the plane the camera cuts it with — and the six standard pump faults are six points in one compression model, not six drawn shapes. |
| `rodpump-geometry` | Rod-pump mechanics: plunger travel off a crank, the fluid load and displacement from the field formulas, the travel at which each ball lifts from an isothermal compression of the gas trapped below the plunger, and the dynamometer card that falls out of the two. |
