...
/** The mark is 28 px in the header; jsdom has no layout, so say so. */
function place(container: HTMLElement, box: { left: number; top: number }) {
  const svg = container.querySelector("svg")!
  svg.getBoundingClientRect = () =>
    ({ ...box, width: 28, height: 28, right: box.left + 28, bottom: box.top + 28 }) as DOMRect
}

/** Long enough for the solver to close a cross-envelope move at 32 units/s. */
...
it("aims at the cursor anywhere on the page", async () => {
